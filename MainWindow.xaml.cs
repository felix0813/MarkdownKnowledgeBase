...
namespace MarkdownKnowledgeBase
{
    public partial class MainWindow : Window
    {
        ...
        private void OnImportNote(object sender, RoutedEventArgs e)
        {
            ...
            var destinationPath = Path.Combine(_rootPath, category, fileName);
            if (File.Exists(destinationPath))
            {
                MessageBox.Show("目标分类下已存在同名笔记。");
                return;
            }
        ...
        private void OnDeleteNote(object sender, RoutedEventArgs e)
        {
            ...
            var result = MessageBox.Show($"确定删除笔记“{_currentNote.Name}”吗？", "确认删除", MessageBoxButton.YesNo, MessageBoxImage.Warning);
            if (result != MessageBoxResult.Yes)
            {
                return;
            }

            var notePath = _currentNote.Path;
            if (File.Exists(notePath))
            {
                File.Delete(notePath);
            }

            var relativeNotePath = GetRelativeNotePath(notePath);
            _metadata.Markers.RemoveAll(marker => string.Equals(marker.NotePath, relativeNotePath, StringComparison.OrdinalIgnoreCase));
            var markerIds = _metadata.Markers.Select(marker => marker.Id).ToHashSet();
            _metadata.Links.RemoveAll(link => !markerIds.Contains(link.SourceMarkerId) || !markerIds.Contains(link.TargetMarkerId));
            SaveMetadata();
